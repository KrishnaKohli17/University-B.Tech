University Journey (2025–2029) | Krishna Kohli

Welcome to my central repository for all academic work completed during my B.Tech at Shoolini University. This repository serves as a comprehensive archive of my growth as a developer, documenting every project, program, and piece of documentation from Semester 1 through Semester 8.

--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

👤 Profile

Name: Krishna Kohli

University: Shoolini University

Batch: 2025 – 2029

Field of Study: Computer Science / Engineering

--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

📂 Repository Structure
The repository is organized by semester. Each semester folder contains subfolders for specific programming languages, projects, and lab documentation.


---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

Year 1

Semester 1: Introduction to Programming, Logic Building, Front-End_Development and Data Science.

Languages: [C, Python, HTML, CSS, JavaScript, MySQL]
Subjects:[Logic Building in Computer Programming,Foundation of DataScience, Basic Aptitude, Front End Development, AI & Future Tech, Generative AI-1]

Highlights: Basic coding, Databasis, Database Managment, Probelm Solving, WebDeveploment, Introduction to [ Artificial Intelligence, Machine Learning, Deep Learning, Neural Network].

Semester 2: Enhancing Computational Logic And Efficiency With Opps and DSA in C/C++, Python and BackEnd.

Languages: [DSA with C/C++, OOPs with C++, Python, JavaScript, Discrete Mathematics]
Subjects:[OOPs using C++ Language,Foundation of DataScience, Basic Aptitude, Front End Development, AI & Future Tech, Generative AI-1]

Highlights:Studying With Discrete Mathematics Along With Transitioning From Basic Coding To Complex Problem-Solving Using DSA (Data Structures & Algorithms),
OOPs (Object Oriented Programs) And Learning Complex Artificial Intelligence,Machine Learning, Deep Learning, Neural Network.

--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

Year 2

Semester 3: XXX.

Languages: [ ]

Highlights: XXX.

Semester 4: XXX.

Languages: [ ]

Highlights: XXX.

--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

Year 3

Semester 5: XXX.

Languages: [ ]

Highlights: XXX.

Semester 6: XXX.

Languages: [ ]

Highlights: XXX.

--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

Year 4

Semester 7: XXX.

Languages: [ ]

Highlights: XXX.


Semester 8: XXX.

Languages: [ ]

Highlights: XXX.

--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

🛠️ Skills Acquired
As I progress through my degree, I am building proficiency in:
Languages: (C, Python, HTML, CSS, JavaScript, MySQL)

Tools: Git, VS Code, Linux, SQL.

Documentation: LaTeX, Markdown, Technical Reporting.

📈 Goals
Maintain a clean and documented codebase for every semester.
Bridge the gap between theoretical university concepts and practical implementation.
Showcase a clear evolution of coding skills over the 4-year period.

📬 Contact
LinkedIn: (https://www.linkedin.com/in/krishna-%E2%80%8E-9a056534b?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app)

Email: [krishna121729@gmai.com]
